...
*/
class Controller_Admin_Abandonados extends Controller_Admin
{
	/**
	* BEFORE
	*
	...
	*/
	public function action_index($search = '')
	{
		...
		if(!empty($sales))
		{
			...
			foreach($sales as $sale)
			{
				...
				$sales_info[] = array(
					...
					'email'         => (!empty($sale->customer->user->email)) ? $sale->customer->user->email : 'N/A',
					'type'          => (!empty($sale->status)) ? $sale->payment->type->name: 'N/A',
		...
		# SE CARGA LA VISTA
		$this->template->title   = 'Carritos Abandonados';
		$this->template->content = View::forge('admin/abandonados/index', $data, false);
	}
	...
	public function action_buscar()
	{
		...
		if(Input::method() == 'POST')
		{
			...
			if($val->run($data))
			{
				...
				Response::redirect('admin/abandonados/index/'.$search);
			}
			else
			{
				# SE REDIRECCIONA AL USUARIO
				Response::redirect('admin/abandonados');
			}
		}
		else
		{
	...
	*/
	public function action_info($sale_id = 0)
	{
		...
		if(!empty($sale))
		{
			...
			$data['type']                     = !empty($sale->payment->type->name) ? $sale->payment->type->name : 'N/A';
		...
		# SE CARGA LA VISTA
		$this->template->title   = 'Información de carrito abandonado';
		$this->template->content = View::forge('admin/abandonados/info', $data);
	}
	...
	public function action_editar_voucher($sale_id = 0)
{
	...
	*/
	public function action_editar($sale_id = 0)
	{
		...
		$data          = array();
		$classes       = array();
		$fields        = array('customer', 'email', 'codigosap', 'transaction','total', 'type', 'name', 'order', 'ordersap', 'factsap', 'guide', 'package', 'id', 'pdf', 'xml');
		$package_opts  = array();
		$order_opts    = array();
		...
		if(!empty($sale))
		{
			...
			$data['type']              = !empty($sale->payment->type->name) ? $sale->payment->type->name : 'N/A';
			$data['type_id']           = !empty($sale->payment->type_id) ? $sale->payment->type_id : 'N/A';
			$data['ordersap']	       = $sale->ordersap;
			$data['factsap']      	   = $sale->factsap;
			$data['guide']      	   = $sale->guide;
		...
		if(Input::method() == 'POST')
		{
			...
			$val->add_callable('Rules');
			$val->add_field('transaction', 'transaction', 'min_length[1]|max_length[255]');
			...
			$val->add_field('status', 'status', 'min_length[1]|max_length[255]');
			...
			if($val->run())
			{
				...
				# SE ESTABLECE LA INFORMACION
				$sale->transaction   	= $val->validated('transaction');
				...
				if($sale->save())
				{
					...
					# SE REDIRECCIONA AL USUARIO
					Response::redirect('admin/abandonados/editar/'.$sale_id);
				}
			}
			else
			{
		...
		# SE CARGA LA VISTA
		$this->template->title   = 'Editar carrito abandonado';
		$this->template->content = View::forge('admin/abandonados/editar', $data);
	}
	...
	*/
    ...
    if (!empty($customers))
    {
        ...
        foreach ($customers as $customer)
        {
            ...
            $customer_opts += array($customer->id => $customer->name. ' '.$customer->last_name);
        }
    }
