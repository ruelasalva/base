...
/**
 * CONTROLADOR ADMIN_BANNERS_LATERALES
 *
 * @package  app
 * @extends  Controller_Admin
 */
class Controller_Admin_Banners_Laterales extends Controller_Admin
{
	/**
	 * BEFORE
	 *
	...
	public function action_index()
	{
		...
		$banners = Model_Banners_Side::query();
		...
		$this->template->content = View::forge('admin/banners_laterales/index', $data, false);
	}
	...
	public function action_agregar()
	{
		...
		if(Input::method() == 'POST')
		{
			...
			if($val->run())
			{
				...
				$order = Model_Banners_Side::query()
				->max('order');

				# SE CREA EL MODELO CON LA INFORMACION
				$banner = new Model_Banners_Side(array(
				...
				if($banner->save())
				{
					...
					# SE REDIRECCIONA AL USUARIO
					Response::redirect('admin/banners_laterales');
				}
			}
			else
			{
		...
		$this->template->content = View::forge('admin/banners_laterales/agregar', $data);
	}
	...
	public function action_info($banner_id = 0)
	{
		...
		$this->template->content = View::forge('admin/banners_laterales/info', $data);
	}
	...
	public function action_editar($banner_id = 0)
	{
		...
		if(Input::method() == 'POST')
		{
			...
			if($val->run())
			{
				...
				if($banner->save())
				{
					...
					# SE REDIRECCIONA AL USUARIO
					Response::redirect('admin/banners_laterales/editar/'.$banner_id);
				}
			}
			else
			{
		...
		$this->template->content = View::forge('admin/banners_laterales/editar', $data);
	}
