...
class Controller_Admin_Crm_Task extends Controller_Admin
{
	...
	*/
	public function action_index($search = '')
	{
		...
		else
		{
			...
			$tasks = $tasks->where('employee_id', -1);
		}
		...
		$this->template->content = View::forge('admin/crm/task/index', $data, false);
	}
	...
	public function action_buscar()
	{
		...
		if(Input::method() == 'POST')
		{
			...
			if($val->run($data))
			{
				...
				Response::redirect('admin/crm/task/index/'.$search);
			}
			else
			{
				# SE REDIRECCIONA AL USUARIO
	...
	*/
	public function action_info($task_id = 0)
	{
		...
		if(!empty($task))
		{
			...
			# SE ALMACENA LA INFORMACION PARA LA VISTA
			$data['user_id']        = $task->employee->name;
		...
		$this->template->content = View::forge('admin/crm/task/info', $data);
	}
	...
	*/
	public function action_agregar()
	{
		...
		$data                = array();
		$classes             = array();
		$fields              = array('id', 'created_at', 'updated_at', 'finish_at' ,'commitment_at' , 'description', 'status_id',  'user_id', 'comments', 'date','time','deleted');
		$date                = date('d/m/Y');
		$time                = date('H:i');
		...
		if(Input::method() == 'POST')
		{
			...
			$employee_id = Model_Employee::query()
			->where('user_id', $user_id)
			->get_one()
			->id;
		...
		# SE CARGA LA VISTA
		$this->template->title   = 'Agregar Tarea Pendiente';
		$this->template->content = View::forge('admin/crm/task/agregar', $data);
	}
	...
	*/
	public function action_editar($task_id = 0)
	{
		...
		$data                = array();
		$classes             = array();
		$fields              = array('id', 'created_at', 'updated_at', 'finish_at' ,'commitment_at' , 'description', 'status_id',  'user_id', 'comments', 'date','time');
		$date                = date('d/m/Y');
		$time                = date('H:i');
		...
		if(Input::method() == 'POST')
		{
			...
			$val->add_callable('Rules');
			//$val->add_field('description', 'Descripción', 'required');
			$val->add_field('comments', 'comentarios', 'required');
			$val->add_field('date', 'fecha de publicación', 'required|date');
		...
		$this->template->content = View::forge('admin/crm/task/editar', $data, false);
	}
