...
class Model_Quotes_Tax_Datum extends \Orm\Model
{
    ...
    {
        $response = new Model_Quotes_Tax_Datum($request);

        return ($response->save()) ? $response : false;
    }

	public static function do_update($request, $quote_id)
	{
		$response = Model_Quotes_Tax_Datum::query()->where('quote_id', $quote_id)->get_one();
        $response->set($request);

		return ($response->save()) ? $response : false;
	}

	public static function do_delete($quote_id)
    {
        $response = Model_Quotes_Tax_Datum::query()->where('quote_id', $quote_id)->get_one();
	...
	protected static $_table_name = 'quotes_tax_data';
