...
<div class="card mt-4">
    <div class="card-header bg-gradient-primary text-white">
        <h3 class="mb-0">Permisos de Usuario: <?php echo $usuario->username; ?> <small>(<?php echo $usuario->email; ?>)</small></h3>
    </div>
    <div class="card-body">
        <form method="post" action="">
            <div class="table-responsive">
                <table class="table table-bordered table-sm align-middle">
                    <thead class="thead-light">
                        <tr>
                            <th rowspan="3" class="align-middle text-center">Módulo</th>
                    ...
                    <tbody>
                        <tr>
                            <td>
                                ...
                                <br><small><?php echo $usuario->email; ?></small>
                            </td>
                            <?php foreach ($modules as $mod_key => $mod): ?>
                                    ...
                                    <?php foreach ($mod['children'] as $child_key => $child_name): ?>
                                        <?php foreach (['view', 'edit', 'delete', 'create'] as $action): ?>
                                            <td class="text-center">
                                                <input type="checkbox"
                                                    name="perm[<?php echo $child_key; ?>][<?php echo $action; ?>]"
                                                    value="1"
                                                    <?php if (isset($perms[$child_key][$action]) && $perms[$child_key][$action]): ?>
                                                        checked
                                                    <?php endif; ?>
                                                >
                                            </td>
                                ...
                                <?php else: ?>
                                    <?php foreach (['view', 'edit', 'delete', 'create'] as $action): ?>
                                        <td class="text-center">
                                            <input type="checkbox"
                                                name="perm[<?php echo $mod_key; ?>][<?php echo $action; ?>]"
                                                value="1"
                                                <?php if (isset($perms[$mod_key][$action]) && $perms[$mod_key][$action]): ?>
                                                    checked
                                                <?php endif; ?>
                                            >
                                        </td>
            ...
            <div class="text-right mt-3">
                ...
                <a href="<?php echo Uri::create('admin/configuracion/permisos/usuario'); ?>" class="btn btn-secondary">Regresar</a>
            </div>
        </form>
    </div>
</div>
