...
<div id="cotizacion-edit-app" class="container-fluid px-4 py-3">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
          ...
          <div v-if="vendedorAsignado" class="alert alert-info py-2 mt-2">
              ...
              Vendedor asignado: <b>{{ vendedorAsignado.name }}</b>
          </div>
      </div>
        ...
        <div class="card-body">
            <div class="form-row">
                ...
                <div class="form-group col-md-3">
                    ...
                    <select v-model="seller_asig_id" class="form-control">
                      <option v-for="emp in employees_opts" :key="emp.id" :value="emp.id">
                        {{ emp.name }}
                      </option>
                    </select>
                    <span class="help-block">Vendedor asignado a la cotización.</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col text-right d-flex flex-wrap justify-content-end align-items-center">
        ...
        <div class="btn-group mb-2 mr-2">
            ...
            <div class="dropdown-menu dropdown-menu-right">
                <button class="dropdown-item" @click="finalizarCotizacion('nuevo')">Guardar y Nuevo</button>
                <button class="dropdown-item" @click="finalizarCotizacion('ver')">Guardar y ver</button>
                <button class="dropdown-item" @click="finalizarCotizacion('cerrar')">Guardar y cerrar</button>
            </div>
        </div>
...
<script>
    // Variables globales unificadas para la aplicación Vue
    ...
    window.cotizacionEdicion = <?= isset($cotizacionEdicion) ? json_encode($cotizacionEdicion) : 'null' ?>;
    window.pending_id = '<?= Input::get('pending_id', ''); ?>';

    // Este código ahora se ejecutará después de que Vue y el DOM estén listos.
    document.addEventListener('DOMContentLoaded', (event) => {
        // Mostrar el botón solo si hay cotizaciones pendientes
        localforage.getItem('cotizaciones_offline').then(cotis => {
            const btnSincronizar = document.getElementById('btn-sincronizar-cotizaciones');
            if (cotis && cotis.length && btnSincronizar) {
                ...
                btnSincronizar.onclick = function() {
                    window.sincronizarCotizacionesOffline(function(status, enviadas, errores) {
                        ...
                        if (status === 'completado') {
                            ...
                            msg = (enviadas ? 'Se enviaron ' + enviadas + ' cotizaciones. ' : '') +
                                  (errores ? 'Hubo ' + errores + ' errores. ' : '');
                            Swal.fire('Sincronización parcial', msg, 'warning');
                        } else {
                            Swal.fire('Error', 'No se pudo sincronizar. Intenta de nuevo.', 'error');
                        }
                    });
                };
            }
...
<?= Asset::js('admin/cotizaciones-editar-vue.js'); ?>