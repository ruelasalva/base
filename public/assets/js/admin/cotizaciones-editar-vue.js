if (document.getElementById('cotizacion-edit-app')) {
    ...
    new Vue({
        ...
        data: {
            ...
            cotizacionEdicion: null, // Se asigna al cargar la vista
            usuarioId: null,
            modoEdicion: window.modoEdicion || false,
            cotizacionId: null, // Se asigna al cargar la vista
        ...
        mounted() {
            ...
            if (window.cotizacionEdicion) {
                this.cotizacionEdicion = window.cotizacionEdicion;
                this.cotizacionId = this.cotizacionEdicion.id; // Asigna el ID del objeto
                this.cargarDatosIniciales();
                this.cargarEmpleadosCompleto();
            }
            ...
            if (window.cotizacionEdicion.quotes_address) {
                this.quotes_address = window.cotizacionEdicion.quotes_address;
                this.address_id = this.quotes_address.id;

                const yaExiste = this.addresses_opts.find(a => a.id === this.quotes_address.id);
                if (!yaExiste) {
                    this.addresses_opts.push({
                        id: this.quotes_address.id,
                        text: this.quotes_address.iddelivery || 'Domicilio registrado'
                    });
                }
            }
        ...
        computed: {
            ...
            iva() {
                ...
                for (let item of this.partidas) {
                    ...
                    // Calcula el precio neto (precio sin impuesto) después del descuento
                    const precioNetoPartida = (item.unit_price * item.quantity) - ((item.unit_price * item.quantity) * (item.discount / 100));

                    // Aplica el impuesto sobre el precio neto
                    total += precioNetoPartida * (tasa / 100);
                }
                return total;
            },
            retencion() {
                ...
                for (let item of this.partidas) {
                    ...
                    // Aplica la retención sobre el precio neto
                    total += precioNetoPartida * (tasa / 100);
                }
                return total;
            },
        ...
        watch: {
            ...
            partner_id(val) {
                this.buscarContactos();
                this.buscarDirecciones(); // Asegúrate de que esta línea esté presente
                this.obtenerProductosConPrecio();
                this.autoguardarCotizacion();
            },
        ...
        methods: {
            // En tu sección methods: { ... }
            async cargarDatosIniciales() {
                if (this.cotizacionEdicion) {
                    const coti = this.cotizacionEdicion;

                    // Mapea los datos del JSON a las propiedades del data
                    this.socioSeleccionado = coti.partner;
                    this.partidas = coti.partidas;
                    this.partner_id = coti.partner_id;
                    this.partner_contact_id = coti.partner_contact_id;
                    this.reference = coti.reference;
                    this.valid_date = coti.valid_date;
                    this.address_id = coti.address_id;
                    this.payment_id = coti.payment_id;
                    
                    this.comments = coti.comments;
                    this.tax_general = coti.tax_general;
                    this.retention_general = coti.retention_general;
                    this.moneda_id = coti.moneda_id;
                    this.discount_general = coti.discount_general_id;

                    this.employee_id = coti.employee_id || null;
                    this.seller_asig_id = null;

                    // Asegurar que el empleado de la cotización esté en la lista
                    if (coti.employee) {
                        const existeEmp = this.employees_opts.find(e => e.id === coti.employee.id);
                        if (!existeEmp) {
                            this.employees_opts.push(coti.employee);
                            console.log('[DEBUG] Empleado insertado manualmente:', coti.employee);
                        }
                    }

                    // Si viene del backend el objeto seller_asig
                    if (coti.seller_asig) {
                        const existeSeller = this.employees_opts.find(e => e.id === coti.seller_asig.id);
                        if (!existeSeller) {
                            this.employees_opts.push({
                                id: coti.seller_asig.id,
                                name: coti.seller_asig.name
                            });
                            console.log('[DEBUG] Vendedor agregado a employees_opts:', coti.seller_asig);
                        }
                    }

                    // Reordenar lista y preseleccionar en el select
                    if (coti.seller_asig_id) {
                        this.employees_opts = [
                            ...this.employees_opts.filter(e => e.id === coti.seller_asig_id),
                            ...this.employees_opts.filter(e => e.id !== coti.seller_asig_id)
                        ];

                        this.seller_asig_id = coti.seller_asig_id;
                        console.log('[DEBUG] Vendedor asignado preseleccionado:', this.seller_asig_id);
                    } else {
                        this.seller_asig_id = null;
                        console.log('[DEBUG] No había vendedor asignado en la cotización');
                    }

                    // Primero intenta con el objeto local (coti)
                    let addressData = coti.quotes_address;

                    // Si no existe, intenta con el objeto global (cotizaciones)
                    if (!addressData && typeof cotizaciones !== 'undefined' && cotizaciones.quotes_address) {
                        addressData = cotizaciones.quotes_address;
                    }

                    if (addressData) {
                        this.addresses_opts.push({
                            id: addressData.id,
                            text: `${addressData.street} ${addressData.number || ''}, ${addressData.colony || ''}, ${addressData.city || ''}, ${addressData.state || ''}`,
                        });

                        this.camposEntregaActivos = true;
                        this.calle = addressData.street;
                        this.numero = addressData.number;
                        this.numero_interior = addressData.internal_number;
                        this.colonia = addressData.colony;
                        this.ciudad = addressData.city;
                        this.estado_id = addressData.state_id;
                        this.cp = addressData.zipcode;
                        this.detalles = addressData.details;
                    } else {
                        console.log('[DEBUG] No se encontró domicilio para la cotización');
                    }


                    // Llama a las demás funciones que no dependen de la lista de direcciones
                    this.buscarContactos(this.partner_id);
            ...
            // ============================================
            // BÚSQUEDA Y CARGA DE DIRECCIONES (ONLINE/OFFLINE)
            // ============================================
            async buscarDirecciones(partnerId) {
                if (!partnerId) {
                    this.address_options = [];
                    this.address_id = null;
                    this.limpiarCamposEntrega();
                    return;
                }

                this.loadingAddresses = true;
                try {
                    const res = await axios.get(this.urlBase + 'get_partner_addresses/' + partnerId, {
                        headers: { 'X-Requested-With': 'XMLHttpRequest' }
                    });
                    ...
                    if (res.data.msg === 'ok') {
                        ...
                        this.address_options = [];
                        console.error('Error al cargar direcciones:', res.data.msg);
                    }
                } catch (error) {
                    this.address_options = [];
            ...
            // ==============================
            // EMPLEADOS (cargar todos en edición)
            // ==============================
            async cargarEmpleadosCompleto() {
                try {
                    console.log('[DEBUG] Iniciando carga de empleados completos...');
                    let res = await axios.post(this.urlBase + 'catalogos_cotizaciones_completo', {
                        access_id: this.access_id,
                        access_token: this.access_token
                    }, {
                    ...
                    if (res.data.msg === 'ok') {
                        ...
                        console.log('[DEBUG] Empleados cargados (completo):', this.employees_opts.length);
                        console.log('[DEBUG] Primer empleado:', this.employees_opts[0] || 'ninguno');
                    } else {
                        console.warn('[WARN] No se pudieron cargar empleados:', res.data.msg);
                    }
                } catch (e) {
                    console.error('[ERROR] cargarEmpleadosCompleto:', e);
                }
            },
            ...
            agregarProducto() {
                ...
                if (existe) {
                    existe.quantity += parseInt(this.nuevo_quantity);
                } else {
                    this.partidas.push({
            ...
            finalizarCotizacion(accion) {
                ...
                // Normalizar partidas antes de enviarlas
                const productosNormalizados = this.partidas.map(p => ({
                    quote_product_id: p.quote_product_id || null,   // id en quotes_products
                    product_id: p.product_id || (p.product ? p.product.id : null), // id real de products
                    code: p.code || (p.product ? p.product.code : ''),
                    name: p.name || (p.product ? p.product.name : ''),
                    quantity: p.quantity,
                    unit_price: p.unit_price,
                    discount: p.discount || 0,
                    image: p.image || (p.product ? p.product.image : ''),
                    tax_id: p.tax_id || (p.tax ? p.tax.id : ''),
                    retention_id: p.retention_id || (p.retention ? p.retention.id : '')
                }));
                ...
                const payload = {
                    ...
                    access_token: this.access_token,
                    partner_id: this.partner_id,
                    quote_id: this.cotizacionEdicion.id,
                    products: JSON.stringify(productosNormalizados),  // ✅ ahora incluye quote_product_id
                    comments: this.comments,
                    payment_id: this.payment_id,
                    seller_asig_id: this.seller_asig_id,
                    partner_contact_id: this.partner_contact_id,
                    reference: this.reference,
                    valid_date: this.valid_date,
                };
                ...
                if (this.seller_asig_id) payload.seller_asig_id = this.seller_asig_id;
//                if (this.employee_id) payload.seller_asig_id = this.employee_id;
                ...
                // AÑADE ESTA LÍNEA AQUÍ MISMO, ANTES DEL ENVÍO.
                if (this.modoEdicion && this.cotizacionId) {
                    payload.quote_id = this.cotizacionId;
                    console.log('[DEBUG] Enviando quote_id para edicion:', this.cotizacionId);
                }
                ...
                axios.post(this.urlBase + 'finalizar_edicion', payload, {
                    headers: { 'X-Requested-With': 'XMLHttpRequest' }
                }).then(res => {
                    if (res.data.msg === 'ok') {
                        ...
                        if (accion === 'nuevo') {
                            ...
                            Swal.fire('¡Guardada!', 'La cotización quedo registrada, puedes capturar una nueva.', 'success');
                            this.resetCampos();
                            this.cotizacionGuardada = false;
                        }

                        if (accion === 'ver') {
                            // Habilita botón de correo, muestra mensaje de éxito
                            Swal.fire('¡Guardada!', 'Cotización registrada. Ahora puedes enviarla por correo o editarla.', 'success');
                            this.cotizacionGuardada = true;
                            // Aquí puedes dejar los campos editables o sólo habilitar el modal de correo
                            // Si quieres que se pueda seguir editando en la misma vista, NO redirijas,
                            // solo pon los datos en el formulario (si tu flujo lo requiere)
                            // Si quieres cargar la info de la cotización, llama a tu método de cargar info aquí.
                        }
